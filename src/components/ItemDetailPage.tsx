...
export default function ItemDetailPage() {
  ...
  const fetchItemDetails = async (itemId: string) => {
    try {
      ...
      if (response.errorCode === 0 && response.data) {
        ...
        const foundItem = mapped.find(item => item.id.toString() === itemId);
  ...
  useEffect(() => {
    if (item && categories.length > 0) {
      const itemCats = categories.filter(cat => item.categoryIds.includes(cat.id));
    ...
    <div className="space-y-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {itemCategories.map((category) => (
                    <span key={category.id} className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <span className="text-gray-600">{itemCategories.length} assigned</span>
        </div>
