...
import { Search, Plus, Eye, Edit, Trash2, Filter, Download, User, Phone, Mail, Calendar, ToggleLeft, ToggleRight, Loader, RefreshCw } from 'lucide-react';
...
export default function DeliverymanListPage() {
  ...
  const loadDeliverymen = async () => {
    try {
      ...
      if (response.errorCode === 0 && response.data) {
        ...
        const drivers = response.data.filter(user => user.role_name === 'driver');
        ...
        const mappedDrivers: DeliverymanData[] = drivers.map(driver => ({
          ...
          address: `${driver.street_address1}${driver.street_address2 ? ', ' + driver.street_address2 : ''}, ${driver.city}, ${driver.state} ${driver.zip_code}`,
          joiningDate: driver.created_at ? new Date(driver.created_at).toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'short',
            day: '2-digit',
            hour: '2-digit',
      ...
      setError(error instanceof Error ? error.message : 'Failed to load delivery drivers');
    } finally {
      setLoading(false);
    }
  };

  const handleStatusToggle = async (driverId: number, currentStatus: 'active' | 'inactive') => {
    try {
      ...
      // Update local state
      setDeliverymen(prev => prev.map(driver => 
        driver.id === driverId 
  ...
  const handleDeleteDriver = async (driverId: number) => {
    if (!confirm('Are you sure you want to delete this delivery driver? This action cannot be undone.')) {
      return;
    }
  ...
  const filteredDeliverymen = deliverymen.filter(deliveryman => {
    ...
    const matchesDate = !joiningDate || deliveryman.joiningDate.includes(joiningDate);
    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-800 mb-2">Loading Delivery Drivers</h3>
          <p className="text-gray-600">Please wait while we fetch the delivery team...</p>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          {deliverymen.length > 0 ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredDeliverymen.map((deliveryman, index) => (
                    <tr key={deliveryman.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button 
                          onClick={() => handleStatusToggle(deliveryman.id, deliveryman.status)}
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-medium text-gray-800 mb-2">No Delivery Drivers Found</h3>
